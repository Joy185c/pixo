...
function DeviceFilesView({ session, userId, onBack }) {
  ...
  const fetchFiles = useCallback((category) => {
    ...
    let path = category
      ? `/sessions/${session.id}/files?category=${category}&limit=5000&offset=0`
      : `/sessions/${session.id}/files?limit=5000&offset=0`
  ...
  const loadMore = useCallback(() => {
    ...
    let path = selectedCat
      ? `/sessions/${session.id}/files?category=${selectedCat}&limit=5000&offset=${curFiles.length}`
      : `/sessions/${session.id}/files?limit=5000&offset=${curFiles.length}`
    if (userId) path = `/admin/users/${userId}` + path
    else        path = `/dashboard` + path
